...
| `converge.mjs --record` exits 2 with `REFUSING TO RECORD` | The one exit 2 that is a refusal to WRITE rather than an inability to read. The ledger it was about to write is one every later run would refuse — a `fixCommit`, `--at` or `--base` that resolves nowhere, or a batch past the entry cap — and the ledger is append-only with no repair path, so it wrote **nothing** and the iteration counter did not advance. Correct the input it names (`decisions.json` is a per-iteration file meant to be corrected by hand; `--at` and `--base` are arguments) and record again. Still not a claim about the review. |
