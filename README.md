...
Because it is off by default, the report says so rather than leaving the reader to infer it. A report with no probe on any finding comes from four different runs — one that never offered reproductions, one that offered them and recorded none, one where reviewers attached some and execution was never enabled, and one that ran them and reproduced nothing — and only the last means the panel tried. All three renderings name which, beside the skipped lanes and the depth, from the plan's policy and the probe record's own flag; a run that recorded neither says nothing rather than guessing.
...
Each entry also records what closed it and who reported it, and those are two separate fields because they came from two places. The commit is per decision, supplied by the fix agent — only the party that wrote the commits knows which one closed which finding. The reporting lanes are *derived* from the report the decision answers, at an identical-title match and nothing weaker. Together they answer the question Phase 9 turns on: which lanes reported the findings this fix commit closed, and therefore must not review it. That list used to be typed on the command line by the orchestrator that had just written the commit, which is the one selection an interested party must not make; `--closed-by-ledger` derives it instead, and the pass's own `reason` says which of the two it was, because an artifact that words them identically cannot be read for either.
