...
Review → fix → verify → repeat, stopping when nothing is both credible enough (cross-validated or consensus) and consequential enough (not advisory, not `info`) to hold the change open — and when no blocking finding is left that nobody ever went on record about. That second condition is not a footnote: a cross-reviewer's own added finding has no validators by construction, so the credibility test alone quietly deleted exactly the findings a cross-review exists to produce. Three pieces make that work, none of which puts a model in the loop:
